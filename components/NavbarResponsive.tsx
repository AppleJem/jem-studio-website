        ...
        <ul className="flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg md:flex-row md:items-center md:space-x-8 md:mt-0 md:border-0  dark:border-gray-700">
          {navLinks.map((link) => (
            <li key={link.link} className="flex justify-center align-center">
              <a href={link.link} className={`${link.link === pathname ? "border-b-stone-50" : ""} ${navbarTransparent ? "md:hover:text-yellow-200" : "md:hover:border-b hover:border-stone-600"} transition-colors ease-out duration-300 border-b border-transparent block py-2 hover:bg-gray-100 md:hover:bg-transparent md:p-0`}>
                {link.text}
              </a>
            </li>
          ))}
          <li className={`py-2 flex justify-center align-center ${navbarTransparent && !props.fixedColor ? "md:bg-yellow-200 border-transparent" : " md:text-stone-700 md:bg-yellow-200 "} md:text-stone-700 md:font-semibold md:px-6 md:rounded-full`}>
            <a href="#contact-form" className={`transition-colors ease-out duration-300 block hover:bg-gray-100 md:hover:bg-transparent md:p-0`}>Contact Us</a>
          </li>
        </ul>
      </div>
    </div>
